...
function registerTextAngularTool(name, toolDefinition) {
    if (!name || name === '' || taTools.hasOwnProperty(name)) throw ('angularjstexteditor Error: A unique name is required for a Tool Definition');
    if (
        (toolDefinition.display && (toolDefinition.display === '' || !validElementString(toolDefinition.display))) ||
        (!toolDefinition.display && !toolDefinition.buttontext && !toolDefinition.iconclass)
    )
        throw ('angularjstexteditor Error: Tool Definition for "' + name + '" does not have a valid display/iconclass/buttontext value');
    taTools[name] = toolDefinition;
}

angular.module('angularjstexteditorSetup', [])
    ...
    .run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', 'taToolFunctions', '$sanitize', '$sce', 'taOptions', '$log',
        function (taRegisterTool, $window, taTranslations, taSelection, taToolFunctions, $sanitize, $sce, taOptions, $log) {
            ...
            var blockJavascript = function (link) {
                if (link && link.toLowerCase().indexOf('javascript') !== -1) return true;
                if (!link) return true;
                return false;
            };
            ...
            taRegisterTool('insertVideo', {
                ...
                onElementSelect: {
                    element: 'iframe',
                    onlyWithAttrs: ['class="ta-iframe-video"'],
                    action: taToolFunctions.imgOnSelectAction
                }
            });
