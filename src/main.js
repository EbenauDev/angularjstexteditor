...
var angularjstexteditor = angular.module("angularjstexteditor",
    ['angularjstexteditorRangy',
        ...
        'angularjstexteditor.validators',
        'angularjstexteditor.taBind']);
            ...
            link: function (scope, element, attrs, ngModel) {
                ...
                angular.extend(scope, angular.copy(taOptions), {
                    ...
                    wrapSelection: function (command, opt, isSelectableElementTool) {
                        ...
                        /* NOT FUNCTIONAL YET */
                        /* angularjstexteditorManager.restoreFocusSelection(scope._name, scope); */
                ...
                _focusin = function (e) {
                    ...
                    /*******  NOT FUNCTIONAL YET
                                        if (e.target.id === 'taTextElement' + _serial) {
                                            ...
                                            angularjstexteditorManager.restoreFocusSelection(scope._name);
                                        }
                    *******/
                ...
                _focusout = function (e) {
                    ...
                    try {
                        ...
                        if (_s) {
                            ...
                            angularjstexteditorManager.saveFocusSelection(scope._name, _savedFocusRange);
                        }
                    } catch(error) { }
                    *****************/
                ...
                // if no ngModel, then the only input is from inside angularjstexteditor
                if (attrs.ngModel) {
                    var _firstRun = true;
                ...
                if (attrs.taTargetToolbars) {
                    ...
                    _editorFunctions = angularjstexteditorManager.registerEditor(scope._name, scope, ['angularjstexteditorToolbar' + _serial]);
                }

                scope.$on('$destroy', function () {
                    angularjstexteditorManager.unregisterEditor(scope._name);
...
angularjstexteditor.service('angularjstexteditorManager', ['taToolExecuteAction', 'taTools', 'taRegisterTool', '$interval', '$rootScope', '$log',
    function (taToolExecuteAction, taTools, taRegisterTool, $interval, $rootScope, $log) {
        // this service is used to manage all angularjstexteditor editors and toolbars.
        ...
        return {
            ...
            registerEditor: function (editorName, editorScope, targetToolbars) {
                ...
                if (!editorName || editorName === '') throw ('angularjstexteditor Error: An editor requires a name');
                if (!editorScope) throw ('angularjstexteditor Error: An editor requires a scope');
                if (editors[editorName]) throw ('angularjstexteditor Error: An Editor with name "' + editorName + '" already exists');
                editors[editorName] = {
                    scope: editorScope,
            ...
            registerToolbar: function (toolbarScope) {
                if (!toolbarScope) throw ('angularjstexteditor Error: A toolbar requires a scope');
                if (!toolbarScope.name || toolbarScope.name === '') throw ('angularjstexteditor Error: A toolbar requires a name');
                if (toolbars[toolbarScope.name]) throw ('angularjstexteditor Error: A toolbar with name "' + toolbarScope.name + '" already exists');
            ...
            updateToolbarToolDisplay: function (toolbarKey, toolKey, _newTool) {
                ...
                else throw ('angularjstexteditor Error: No Toolbar with name "' + toolbarKey + '" exists');
            },
            ...
            // this is used when externally the html of an editor has been changed and angularjstexteditor needs to be notified to update the model.
            ...
            refreshEditor: function (name) {
                ...
                } else throw ('angularjstexteditor Error: No Editor with name "' + name + '" exists');
                touchModification();
            },
            ...
            // return the current version of angularjstexteditor in use to the user
            getVersion: function () { return angularjstexteditorVersion; },
            // for testing
...
angularjstexteditor.directive('angularjstexteditorToolbar', [
    '$compile', 'angularjstexteditorManager', 'taOptions', 'taTools', 'taToolExecuteAction', '$window',
    function ($compile, angularjstexteditorManager, taOptions, taTools, taToolExecuteAction, $window) {
        return {
            scope: {
            ...
            link: function (scope, element, attrs) {
                if (!scope.name || scope.name === '') throw ('angularjstexteditor Error: A toolbar requires a name');
                angular.extend(scope, angular.copy(taOptions));
                ...
                scope.updateToolDisplay = function (key, _newTool, forceNew) {
                    ...
                    if (toolInstance) {
                        ...
                        if (_newTool.buttontext === null && _newTool.iconclass === null && _newTool.display === null)
                            throw ('angularjstexteditor Error: Tool Definition for updating "' + key + '" does not have a valid display/iconclass/buttontext value');
                ...
                angularjstexteditorManager.registerToolbar(scope);

                scope.$on('$destroy', function () {
                    angularjstexteditorManager.unregisterToolbar(scope.name);
                });
            }
        };
    }
]);
angularjstexteditor.directive('angularjstexteditorVersion', ['angularjstexteditorManager',
    function (angularjstexteditorManager) {
        var version = angularjstexteditorManager.getVersion();
        return {
            restrict: "EA",
            link: function (scope, element, attrs) {
